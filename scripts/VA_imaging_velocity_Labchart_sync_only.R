...
sync_thresh <- df_screen |>  
  dplyr::filter(id == id0, session == visit0, protocol == protocol0) |>  
  pull(sync_threshold)
# manual override sync_thresh to define point
#sync_thresh <- 2
df_cine$rolling_zeros <- rollmean(df_cine$peak_positive_cm_s, k = 200, fill = 0, align = "left")
zeros <- which(df_cine$rolling_zeros < sync_thresh)
...
df_cine$time_adj[frame_now] <- c(df_cine$time_seconds[frame_now[1]], 
                                 ...
                                 (df_cine$time_seconds[frame_now[1]] + frame_time*3/4))
frame_xdist <- mean(diff(df_cine$x_location[frame_now]), na.rm = F)
if (abs(frame_xdist) > 10) df_cine[frame_now, 5:16] <- NA
...
hypocapnic_dfv <- df_screen |>  
  dplyr::filter(id == id0, session == visit0, protocol == protocol0) |>  
  pull(hypocapnic_dfv)

hypocapnic_sfv <- df_screen |>  
  dplyr::filter(id == id0, session == visit0, protocol == protocol0) |>  
  pull(hypocapnic_sfv)

threshold_df <- tibble(y = c((peak_dfv_thresh-5), 
                             peak_dfv_thresh, 
                             (peak_dfv_thresh+5), 
                             (peak_dfv_thresh+10),
                             (peak_sfv_thresh-5), 
...
if (protocol0 %in% c("HYPOCVR", "REBREATHE", "LBNPINC")){
  ...
  print(df_clean |> 
          plot_ly(type = "scatter",
                  mode = "lines") |> 
          add_trace(x = ~time_s, y = ~peak_positive_cm_s, 
                    line = list(color = "black"), name = "raw") |> 
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       y = threshold_df$y, yend = threshold_df$y,
                       line = list(colour = "darkgreen", width = 1),
                       name = "5 cm/s increments") |> 
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       y = peak_dfv_thresh, yend = peak_dfv_thresh,
                       line = list(color = "purple", width = 3),
                       name = "Current diastolic threshold") |>
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       y = peak_sfv_thresh, yend = peak_sfv_thresh,
                       line = list(color = "red", width = 3),
                       name = "Current systolic threshold") |>
          layout(shapes = list(type = "rect",
                 ...
                 yaxis = list(title = "Peak Velocity (cm/s)", fixedrange = F)) |> 
          rangeslider(0,20))
  ...
  message(str_c("The recorded peak dfv threshold (within the shaded hypocapnic period) is", hypocapnic_dfv, sep = " "))
  ...
  if (change_thresh == "Y" | change_thresh == "y") hypocapnic_dfv <- as.numeric(readline("What is estimated DFV threshold for peak signal (within the shaded hypocapnic period)? > "))

  message(str_c("The recorded peak sfv threshold (within the shaded hypocapnic period) is", hypocapnic_sfv, sep = " "))
  change_thresh <- readline("Based on the plot of peak velocity (within the shaded hypocapnic period). Do you wish to change this threshold (y/n)? > ")
  if (change_thresh == "Y" | change_thresh == "y") hypocapnic_sfv <- as.numeric(readline("What is estimated SFV threshold for peak signal (within the shaded hypocapnic period)? > "))
  ...
  print(df_clean |> 
          ...
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       ...
                       line = list(color = "red", width = 3),
                       name = "Current systolic threshold") |>
          layout(xaxis = list(title = "Time (s)"),
                 yaxis = list(title = "Peak Velocity (cm/s)", fixedrange = F)) |> 
          rangeslider(0,20))
...
if (protocol0 %in% c("HYPOCVR", "REBREATHE", "LBNPINC")){
  ...
  print(df_clean |> 
          plot_ly(type = "scatter",
                  mode = "lines") |> 
          add_trace(x = ~time_s, y = ~mean_positive_cm_s, 
                    line = list(color = "black"), name = "raw") |> 
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       y = threshold_df$y[1:4], yend = threshold_df$y[1:4],
                       name = "5 cm/s increments") |> 
          add_segments(x = min(df_clean$time_s), xend = max(df_clean$time_s), 
                       y = mean_dfv_thresh, yend = mean_dfv_thresh,
                       line = list(color = "purple", width = 3),
                       name = "Current diastolic threshold") |>
          layout(shapes = list(type = "rect",
                               fillcolor = "skyblue",
                               ...
                               layer = "below"),
                 xaxis = list(title = "Time (s)"),
                 yaxis = list(title = "Mean Velocity (cm/s)", fixedrange = F)) |> 
          rangeslider(0,20))
...
if (protocol0 %in% c("HYPOCVR", "REBREATHE", "LBNPINC")){
  ...
  peak_sfv_noise3 <- which(df_clean$peak_positive_cm_s[t1:t2] > hypocapnic_sfv) + t1 -1 
  peak_dfv_noise3 <- which(df_clean$peak_positive_cm_s[t1:t2] < hypocapnic_dfv) + t1 -1 
  ...
  noise <- sort(unique(vctrs::vec_c(peak_dfv_noise, peak_sfv_noise, mean_dfv_noise,
                                    ...
                                    peak_sfv_noise3, peak_dfv_noise3)))
...
if (protocol0 == "STEP") protocol0 <- "_STEP"
...
if (protocol0 == "_STEP") protocol0 <- "STEP"
...
top_value <- c("TopValue=", "1000")
bottom_value <- c("BottomValue=", "-1000")
data_header <- rbind(data_header, data_units, top_value, bottom_value)
data_header$V3 <-  c("", "", "", "", "Peak_VABV", "10.000V", "cm/s", "1000", "-1000")
data_header$V4 <-  c("", "", "", "", "Mean_VABV", "10.000V", "cm/s", "1000", "-1000")
data_header$V5 <-  c("", "", "", "", "", "", "", "", "")
